...
#define PIN       D3                  // Číslo pinu, který ovládá NeoPixel LED pásek
#define NUMPIXELS 10                  // Počet připojených NeoPixel LED

Adafruit_NeoPixel pixels(NUMPIXELS, PIN, NEO_GRB + NEO_KHZ800);    // Definice objektu "pixels" pro práci s NeoPixel
  
const char* ssid = "Network";         // Nastavení připojení k Wifi
...
void setup() {
  ...
  Serial.begin(115200);
  delay(10);
  Serial.println();                       // Vypisuje na serial monitor údaje o připojování
  Serial.println();
  Serial.print("Connecting to : ");
  Serial.println(ssid); 

  IPAddress staticIP(192, 168, 15, 32);   //ESP static ip
...
void initNeoPixel(){
  ...
  for(int i=0; i<NUMPIXELS; i++) {                            // Cyklus, který postupně nastaví jednotlivé LED
    pixels.setPixelColor(i, pixels.Color(125, 0, 0));         // Nastavení jednotlivé LED na červenou poloviční jas (RGB)
...
Text += "<style>body {font-family: Arial;} .tab {overflow: hidden;border: 1px solid #ccc;background-color: #f1f1f1;} .tab button {background-color: inherit;float: left;border: none;outline: none;cursor: pointer;padding: 10px 8px;transition: 0.3s;font-size: 14px;} .tab button:hover {background-color: #ddd;} .tab button.active {background-color: #FFAA00;color: white} .tabcontent {padding: 6px 12px;border: 1px solid #ccc;border-top: none;background-color: white;} Fieldset {border: 1px solid #FFAA00;color: #FFAA00;position: relative;} input {box-sizing: border-box;margin: 5px;float: left;height: 50px;width: 100px;}</style>";
Text += "</head><body><div class=\"tab\"><button class=\"tablinks\" onClick=\"window.location.href = 'http://192.168.15.30';\">LED TV</button><button class=\"tablinks\" onClick=\"window.location.href = 'http://192.168.15.31';\">LED Postel</button><button class=\"active\" >LED Maska</button><button class=\"tablinks\" onClick=\"window.location.href = 'http://192.168.15.33';\">Termostat</button><button class=\"tablinks\" onClick=\"window.location.href = 'http://192.168.15.34';\">Kamera</button></div>";
...
Text += "<Fieldset><legend>Barva</legend><form action=\"/barva\" method=\"get\"><input type=\"color\" name=\"barva\" value=\"#FF0000\" ><input type=\"submit\" value=\"Nastavit\" ></form></Fieldset><br>";
