...
#define NUMPIXELS 71                  // Počet připojených NeoPixel LED
...
void setup() {
  ...
  IPAddress staticIP(192, 168, 15, 30);     //ESP static ip
...
void initNeoPixel() {
  ...
  for (int i = 0; i < NUMPIXELS; i++) {                       // Cyklus, který postupně nastaví jednotlivé LED
    pixels.setPixelColor(i, pixels.Color(255, 255, 255));     // Nastavení jednotlivé LED na bílou poloviční jas (RGB)
    pixels.show();                                            // Odeslání nastavení do NeoPixel pásku
  }
}

// nastavení akce pro připojení na hlavní stránku (URI : /). Soubor s obsahem HTML je uložen v SPIFFS.
void homePage() {
...
String HTMLStranka () {
  ...
  Text += "<style>body {font-family: Arial;} .tab {overflow: hidden;border: 1px solid #ccc;background-color: #f1f1f1;} .tab button {background-color: inherit;float: left;border: none;outline: none;cursor: pointer;padding: 10px 8px;transition: 0.3s;font-size: 14px;} .tab button:hover {background-color: #ddd;} .tab button.active {background-color: #0080C0;color: white} .tabcontent {padding: 6px 12px;border: 1px solid #ccc;border-top: none;background-color: white;} Fieldset {border: 1px solid #0080C0;color: #0080C0;position: relative;} input {box-sizing: border-box;margin: 5px;float: left;height: 50px;width: 100px;}</style>";
  Text += "</head><body><div class=\"tab\"><button class=\"active\" >LED TV</button><button class=\"tablinks\" onClick=\"window.location.href = 'http://192.168.15.31';\">LED Postel</button><button class=\"tablinks\" onClick=\"window.location.href = 'http://192.168.15.32';\">LED Maska</button><button class=\"tablinks\" onClick=\"window.location.href = 'http://192.168.15.33';\">Termostat</button><button class=\"tablinks\" onClick=\"window.location.href = 'http://192.168.15.34';\">Kamera</button></div>";
